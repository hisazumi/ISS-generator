...
void
address2label (char *label, int n, int address)
{
...
/* offset は 0 が前提 */
void
basic_block_prepare (Function *function, int num)
{
  int pc = 0;
  for (int i=0 ; i<num ; i++)
    {
...
emit_body ()
{
  ...
  /* 命令数分のBBをあらかじめ準備 */
  basic_block_prepare (function, sizeof (insts) / (sizeof (int) * 3));
  ...
  for (int pc=0; it != list.end () ; it++, pc++)
    {
	 ...
	 各BBのアドレスは自明なのでそれを設定している。*/
      ...
      int inst = insts[pc*3];
      int op1  = insts[pc*3+1];
      int op2  = insts[pc*3+2];
      
      switch (inst)
	{
	case LDI:
	  builder.CreateStore (ConstantInt::get(Type::getInt32Ty(context), op2),
			       ir_reg (op1));
	  builder.CreateBr (basic_blocks.at (pc+1));
	  break;
	  
	case ADD:
	  builder.CreateStore (builder.CreateAdd (ir_reg_load (op1),
						  ir_reg_load (op2)),
			       ir_reg (op1));
	  builder.CreateBr (basic_blocks.at (pc+1));
	  break;
	...
	case NE:
	  builder.CreateStore (builder.CreateICmpNE (ir_reg_load (op1),
						     ir_reg_load (op2)),
			       flags);
	  builder.CreateBr (basic_blocks.at (pc+1));
	  break;
	...
	case JC:
	  builder.CreateCondBr (ir_flags_load (),
				basic_blocks.at (op1),
...
{
  program_counter =
    emit_register (Type::getInt32Ty (context), "program_counter", &_program_counter);
  flags =
    emit_register (Type::getInt1Ty (context), "flags", &_flags);
  regs =
    emit_register (ArrayType::get (Type::getInt32Ty (context), 10), "regs", _regs);
  // メモリは?
}
